...
<div className={`fixed top-0  right-0 h-full w-full max-w-md bg-brand-bg z-[70] shadow-2xl transition-transform duration-300 flex flex-col pb-[70px] ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}>        {/* Header */}
        <div className="flex mt-12 items-center justify-between p-5 border-b border-gray-200">
          ...
          <button onClick={() => setCartOpen(false)} className="p-2 text-brand-muted hover:text-brand-cream transition-colors rounded-lg hover:bg-gray-100">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              ...
              <button onClick={() => { setCartOpen(false); navigate('/products'); }} className="bg-black text-white border border-gray-300 font-medium px-6 py-2.5 rounded-lg text-sm hover:bg-white hover:text-black transition-all">
                Browse Products
              </button>
            </div>
          ) : (
            cartItems.map(item => (
              <div key={item.key} className="flex gap-3 p-3 bg-gray-100 rounded-xl border border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => updateQuantity(item.key, item.quantity - 1)} className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-brand-cream hover:border-brand-accent text-sm">−</button>
        ...
        {cartItems.length > 0 && (
          <div className="p-5 border-t  border-gray-200 space-y-3">
            ...
            <button onClick={() => { setCartOpen(false); navigate('/cart'); }} className="w-full bg-black text-white border border-gray-300 font-semibold py-3.5 rounded-xl hover:bg-white hover:text-black transition-all">
              View Cart & Checkout
            </button>
          </div>
        )}
