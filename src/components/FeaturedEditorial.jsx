...
function FeaturedItem({ product, dark }) {
  ...
  const placeholderText = dark ? '#666' : '#aaa';
  ...
  if (!product) {
    return (
      <div>
        ...
        <div style={{
          aspectRatio: '3/4', background: placeholderBg,
          display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '10px',
        }}>
          <span style={{ fontSize: '0.68rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: placeholderText, fontWeight: 600 }}>Add Product</span>
        </div>
      </div>
    );
  }
  return (
    <Link to={`/products/${product._id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
      <div style={{ borderTop: `1px solid ${dividerColor}`, marginBottom: '14px' }} />
      <div style={{ position: 'relative', aspectRatio: '3/4', background: '#f2f2f2', marginBottom: '10px', overflow: 'hidden' }}>
...
export default function FeaturedEditorial({
  ...
  return (
    <section className="featured-editorial-section" style={{ background: dark ? '#000' : '#f2efe9', padding: noTopPadding ? '0 1.5rem 2.5rem' : '2.5rem 1.5rem' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }} className="featured-editorial">
        ...
        <div className="featured-editorial-grid">
          {items.map((p, i) => <FeaturedItem key={p?._id || `ph-${i}`} product={p} dark={dark} />)}
        </div>

        {/* Right: banner image with overlay — light placeholder theme until a real photo is set */}
